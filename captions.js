...
} else if (hrefCheck("oload.download") || hrefCheck("openload.co") || hrefCheck("localhost/player") || hrefCheck("videobot.ddns.net") || hrefCheck("streamango.com")) {
    iDictionaryId = 415092182;
    // iDictionaryId = 401005908;
    iDictionaryLink = "https://ss.lt-center.info/test/" + iDictionaryId + "/";
...
function speakFromUrl(request){
	...
	$.getJSON("https://ss.lt-center.info/test/lingualeo/gettranslates?word=" + request, function(data) {
		console.log(data)
...
function addClickListener() {
    $("#iblock").click(function(event) {
        ...
        if (currentText !== '') {
            ...
            $.getJSON("https://ss.lt-center.info/test/lingualeo/gettranslates?word=" + currentRequestText, function(data) {
                //console.log(data)
                addIntoDictionary(data, currentText.trim());
            });
        }
...
function checkCue(i){
    ...
    var cues = player.textTracks().tracks_[0].cues
	
	if(i<cues.length){
	//if(i<50){
		//console.log("call againt")
		var time = player.textTracks().tracks_[0].cues[i].startTime 
		var currentCue = player.textTracks().tracks_[0].cues[i].text
		var translationsFound = checkDictionary(currentCue,true);
		if(translationsFound && player.currentTime() < time ){
...
function addCuttingsModeListener() {

    $(document).keydown(function(e) {
        if (e.which == 118) {
			checkCue(0);
			    sentence = "CUTTINGS MODE"
				showSubtitles(sentence, false);
        }
    });
}
